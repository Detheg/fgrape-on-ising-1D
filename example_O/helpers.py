...
def generate_random_bloch_state(key, N_qubits):
    ...
    cos_theta = jax.random.uniform(subkey3, minval=0.0, maxval=1.0)
    cos_theta_half = (0.5 * (1.0 + cos_theta))**0.5
    sin_theta_half = (0.5 * (1.0 - cos_theta))**0.5
    phi = jax.random.uniform(subkey2, minval=0.0, maxval=2*jnp.pi)
    alpha = cos_theta_half
    beta = sin_theta_half * jnp.exp(1j * phi)
...
experiments_format = [ # Format of variables as tuples (name, type, required by grape, required by lut, required by rnn)
    ("model", str, True, True, True), # model type
